...
export class TuneInRadio extends MusicPlatform {
	...
	public async getByLink(link: string): Promise<MusicItem> {
		...
		if (matches) {
			...
			const res = await axios.get<any>(`${BASE_URL}profiles/${id}&formats=mp3,aac,ogg`);
	...
	public async search(searchTerm: string, maxResults: number = 10): Promise<MusicItem[]> {
		...
		const res = await axios.get<any>(`${BASE_URL}profiles?fullTextSearch=true&formats=mp3,aac,ogg&query=${search}`);
